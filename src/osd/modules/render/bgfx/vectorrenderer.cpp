...
// including the former default beam-width scale of 0.75.
...
// Restore the reference moving-line median to approximately RGB 189. The
// timed composite stage preserves ordinary luminance ratios and applies its
// display-protection shoulder only near the SDR output ceiling.
...
bgfx_vector_renderer::bgfx_vector_renderer(effect_manager &effects, osd_options const &options)
	...
	, m_available(false)
	, m_present(false)
	, m_colour_monitor(false)
	, m_persistence(0.0f)
	, m_beam_intensity(0.0f)
	, m_untimed_halo(0.0f)
	, m_tail_fraction(0.0f)
...
void bgfx_vector_renderer::draw_beams(uint16_t view, double frame_time)
{
	...
	auto const beam_drive = [have_timing] (render_primitive const &primitive)
	{
		return
				(have_timing && (primitive.vector_intensity >= 0.0F))
						? primitive.vector_intensity
						: primitive.color.a;
	};
	...
	// Timed excitation is calibrated directly by BEAM_ENERGY_RATE_1080. Retain
	// the former user gain only for untimed compatibility rendering.
	...
	while (offset < m_vectors.size())
	{
		...
		for (uint32_t index = 0; index < count; ++index)
		{
			...
			float const drive = beam_drive(primitive);
...
void bgfx_vector_renderer::prepare(uint32_t &view, render_primitive *first, uint16_t width, uint16_t height, double emu_time)
{
	...
	for (render_primitive *primitive = first; primitive; primitive = primitive->next())
	{
		...
		if ((primitive->type == render_primitive::LINE) && PRIMFLAG_GET_VECTOR(primitive->flags))
		{
			...
			// Monochrome vector generators drive all three renderer channels
			// equally. Remember any chromatic, emitting vector so white vectors
			// on a colour monitor retain that monitor's P22 white point.
			float const maximum = std::max({ primitive->color.r, primitive->color.g, primitive->color.b });
			float const minimum = std::min({ primitive->color.r, primitive->color.g, primitive->color.b });
			m_colour_monitor =
					m_colour_monitor ||
					((primitive->color.a > 0.0f) && ((maximum - minimum) > 0.0001f));
		}
	}
...
void bgfx_vector_renderer::composite(uint16_t view)
{
	...
	set_uniform(
			...
			2.2f,
			0.0f);
	// Select the monochrome P4 model or the colour P22 model. Their fixed
	// colourimetric calibrations live in the composite shader.
	set_uniform(
			m_composite_effect,
			"u_phosphor_model",
			m_colour_monitor ? 0.0f : 1.0f,
			have_timing ? 1.0f : 0.0f);
...
void bgfx_vector_renderer::create_sliders()
{
	...
	static constexpr slider_description descriptions[SLIDER_COUNT] =
	{
		...
		{ "Vector untimed beam halo",      0,   4, 100, 1 },   // 0.04
		{ "Vector beam tail fraction",     0,  60, 100, 1 },   // 0.60
		{ "Vector bloom strength",         0,   5,  20, 1 },   // 0.05
...
int32_t bgfx_vector_renderer::slider_changed(slider_id id, std::string *text, int32_t new_value)
{
	...
	switch (id)
	{
		...
		case SLIDER_BEAM_INTENSITY:
			...
			format = "%1.2fx";
			break;
		case SLIDER_UNTIMED_HALO:
			setting = &m_untimed_halo;
			scale = 0.01f;
			format = "%1.2fx";
			break;
		case SLIDER_TAIL_FRACTION:
			setting = &m_tail_fraction;
			scale = 0.01f;
			format = "%1.2f";
			break;
