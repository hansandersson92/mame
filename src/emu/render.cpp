...
void render_container::add_line(float x0, float y0, float x1, float y1, float width, rgb_t argb, u32 flags, float vector_start_time, float vector_ramp_duration, float vector_beam_on_duration, float vector_total_duration, float vector_intensity)
{
	...
	newitem.m_vector_intensity = vector_intensity;
}
	...
	for (render_container::item &curitem : container.items())
	{
		...
		switch (curitem.type())
		{
			case CONTAINER_ITEM_LINE:
				...
				prim->vector_intensity = curitem.vector_intensity();
