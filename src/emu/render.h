...
public:
	...
	float               vector_intensity = -1.0F; // normalized beam current (legacy full scale 255 = 1.0, negative if unavailable)
...
public:
	...
	void add_line(float x0, float y0, float x1, float y1, float width, rgb_t argb, u32 flags, float vector_start_time = -1.0F, float vector_ramp_duration = -1.0F, float vector_beam_on_duration = -1.0F, float vector_total_duration = -1.0F, float vector_intensity = -1.0F);
...
private:
	...
	public:
		item() : m_next(nullptr), m_type(0), m_flags(0), m_internal(0), m_width(0), m_vector_start_time(-1), m_vector_ramp_duration(-1), m_vector_beam_on_duration(-1), m_vector_total_duration(-1), m_vector_intensity(-1), m_texture(nullptr) { }
		...
		float vector_intensity() const { return m_vector_intensity; }
	...
	private:
		...
		float               m_vector_intensity; // normalized beam current (legacy full scale 255 = 1.0, negative if unavailable)
